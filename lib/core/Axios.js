...
class Axios {
   ...
   * 请求方法 所有请求都会走这个方法
   *
   * @param {String|Object} configOrUrl 此请求的特定配置（与 this.defaults 合并）
   * @param {?Object} config
   *
  ...
  request(configOrUrl, config) {
     ...
     * 校验 transitional
     *
     * 检测 1.0.0版本以后 transitional配置将移除
     */
    ...
    if (transitional !== undefined) {
      validator.assertOptions(
        transitional,
        {
          // 忽略 JSON.parse(response.body) 的错误
          silentJSONParsing: validators.transitional(validators.boolean),
          // 当 responseType!== json 时将 response 转化为 json
          forcedJSONParsing: validators.transitional(validators.boolean),
          // 当请求超时返回 ETIMEDOUT 而不是 ECONNABORTED
